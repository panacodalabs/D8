...
(c==0?7:c)):c==0?b:b.addDays(7-c),a=this.getCalendarWeek(),d=d.addDays((a-1)*7),a=[],b=0;b<7;b++)c=d.addDays(b),c=this.create(c.format("mm")+"/"+c.format("dd")+"/"+c.format("yyyy")),a.push(c);return a},create:function(a){var b=typeof a==="number"?a:null;if(!b){if((b=/(\d{1,2})\.(\d{1,2})\.(\d{2,4})/.exec(a))&&b[1]&&b[2]&&b[3])a=a.split(" "),a=b[2]+"/"+b[1]+"/"+b[3]+(a[1]?" "+a[1]:"");b=Date.parse(a)}if(a&&!b)throw"Error: Invalid dateString '"+a+"' passed to create().";else if(!a)return this.now();
a=new D8;a.date=new Date(b);return a}}};D8.getCalendarWeek=function(a,b,c){var d=Math.floor((14-b)/12),a=a+4800-d,b=c+Math.floor((153*(b+12*d-3)+2)/5)+365*a+Math.floor(a/4)-Math.floor(a/100)+Math.floor(a/400)-32045,b=(b+31741-b%7)%146097%36524%1461,c=Math.floor(b/1460);return Math.floor(((b-c)%365+c)/7)+1};
D8.getDatesOfCalendarWeek=function(a,b,c){for(var c=c&&!isNaN(c)?c:this.date?this.format("yyyy"):this.now().format("yyyy"),c=this.create("01/01/"+c),d=c.format("D"),f=null,f=b?d==1?c:c.addDays(8-(d==0?7:d)):d==0?c:c.addDays(7-d),a=a?a:this.getCalendarWeek(),a=f.addDays((a-1)*7),b=[],c=0;c<7;c++)d=a.addDays(c),d=this.create(d.format("mm")+"/"+d.format("dd")+"/"+d.format("yyyy")),b.push(d);return b};
D8.getDateByWeekdayAndCalendarWeek=function(a,b,c){if(a&&!isNaN(a)&&(b&&!isNaN(b)||b===0))if((c=this.getDatesOfCalendarWeek(a,!1,c))&&c.length>0&&c[b])return c[b];else throw"Error: Day "+b+" of calendar week "+a+" could not be found!";else throw"Error: Please pass a valid calendarWeek and a valid day of the week!";};
D8.create=function(a){var b=typeof a==="number"?a:null;if(!b){if((b=/(\d{1,2})\.(\d{1,2})\.(\d{2,4})/.exec(a))&&b[1]&&b[2]&&b[3])a=a.split(" "),a=b[2]+"/"+b[1]+"/"+b[3]+(a[1]?" "+a[1]:"");b=Date.parse(a)}if(a&&!b)throw"Error: Invalid dateString '"+a+"' passed to create().";else if(!a)return this.now();a=new D8;a.date=new Date(b);return a};var \u00d8=D8;